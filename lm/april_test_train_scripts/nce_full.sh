...
model_type=${3}
gen_checkpoint=${4}
# in this script we reuse the training data to train the discriminator, which is probably wrong.
# in the *dev_corrected* scripts we make use of some 'dev' data to train te discriminator instead.
input_file="/checkpoint/kitsing/grover/test_tfrecords/preprocessed_train*.tfrecord" # put your input files here, it can also be something like "*.tfrecord"
...
input_dev_file="/checkpoint/kitsing/grover/test_tfrecords/preprocessed_val*.tfrecord" # put your input files here, it can also be something like "*.tfrecord"
